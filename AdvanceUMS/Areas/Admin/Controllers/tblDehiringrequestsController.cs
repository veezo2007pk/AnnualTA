...
{
    public class tblDehiringrequestsController : Controller
    {
        ...
        public ActionResult DeHire(string   id, int status)
        {
            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var WorkerData = db.tblWorkers.Find(new Guid(id));
                    WorkerData.uqId = new Guid(id);
                    WorkerData.intDehireStatus = status;
                    ...
                    //tblDehireLog tblDehireLog = new tblDehireLog();
                    //tblDehireLog.uqDehireLog = new Guid();
                    //tblDehireLog.intDehireStatus = status;
                    //tblDehireLog.uqWorkerId = new Guid(id);
                    //tblDehireLog.uqCreatedBy = new Guid(dHelper.CurrentLoginUser());
                    //tblDehireLog.dtCreatedAt = DateTime.Now;
                    //db.tblDehireLogs.Add(tblDehireLog);
                    //db.SaveChanges();


                    transaction.Commit();
                    string Description = ("De-hire  for worker - " + WorkerData.uqId);
             ...
             return Redirect("/admin/tblDehiringrequests");
        }


        public ActionResult DeHireCollection(FormCollection formCollection)
        {
            ...
            //{
                ...
                string[] ids = formCollection["ID"].Split(new char[] { ',' });
                    foreach (string id in ids)
                    {
                        ...
                        WorkerData.uqId = new Guid(id);
                        WorkerData.intDehireStatus = 2;
                        WorkerData.uqModifyBy = new Guid(dHelper.CurrentLoginUser());
                        WorkerData.dtMoidifyAt = dateTime;
                        db.Entry(WorkerData).State = EntityState.Modified;
                        db.SaveChanges();

                    string Description = ("De-hire request for worker - " + WorkerData.uqId);
                    dHelper.LogAction(Description);
                    //tblDehireLog tblDehireLog = new tblDehireLog();
                    //tblDehireLog.uqDehireLog = new Guid();
                    //tblDehireLog.intDehireStatus = 2;
                    //tblDehireLog.uqWorkerId = new Guid(id);
                    //tblDehireLog.uqCreatedBy = new Guid(dHelper.CurrentLoginUser());
                    //tblDehireLog.dtCreatedAt = DateTime.Now;
                    //db.tblDehireLogs.Add(tblDehireLog);
        ...
        public ActionResult PopulateSupervisors(string Status)
        {
            ...
            string engineerCode = new Guid(dHelper.CurrentEngineerLoginUser()).ToString();
            ...
            var list = from e in workers
                       ...
                       where e.intDehireStatus==1 && a.uqEngineerCode== new Guid(engineerCode) && e.Year==Status
