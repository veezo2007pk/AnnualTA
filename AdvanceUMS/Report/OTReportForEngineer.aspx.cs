...
{
    public partial class OTReportForEngineer : System.Web.UI.Page
    {
            ...
            protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                using (var _context = new Entities())
                {
                    if (searchText == "")
                    {
                        var summary = _context.OTForEngineer(null).Where(x => x.Year == year).ToList();

                        CustomerListReportViewer.LocalReport.ReportPath = Server.MapPath("~/Report/RDLC/OTReportForEngineer.rdlc");
                    ...
                    {
                        var summary = _context.OTForEngineer(new Guid(searchText)).Where(x => x.Year == year).ToList();
