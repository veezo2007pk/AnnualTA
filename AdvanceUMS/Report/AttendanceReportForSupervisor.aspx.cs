...
{
    public partial class AttendanceReportForSupervisor : System.Web.UI.Page
    {
            ...
            protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                using (var _context = new Entities())
                {
                    if (searchText == "")
                    {
                        var summary = _context.AttendanceForSupervisor(null).Where(x => x.Year == year).ToList();
                        CustomerListReportViewer.LocalReport.ReportPath = Server.MapPath("~/Report/RDLC/AttendanceReportForSupervisor.rdlc");
                    ...
                    {
                        var summary = _context.AttendanceForSupervisor(new Guid(searchText)).Where(x => x.Year == year).ToList();
                        CustomerListReportViewer.LocalReport.ReportPath = Server.MapPath("~/Report/RDLC/AttendanceReportForSupervisor.rdlc");
                        CustomerListReportViewer.LocalReport.DataSources.Clear();
                        ReportDataSource rdc = new ReportDataSource("DataSet1", summary);
                        CustomerListReportViewer.LocalReport.DataSources.Add(rdc);
